...
\title[ECG Classification Comparison]{Comparative Analysis of Machine Learning Architectures\\for ECG Classification}
\subtitle{A Comprehensive Study of Fifteen Approaches Including Deep Learning and Probabilistic Models}
...
\begin{enumerate}
    ...
    \item Implement \textbf{Liquid Time-Constant Network (LTC)} for continuous-time ECG modeling
    \item Implement \textbf{Hidden Markov Models (HMM)} and \textbf{Hierarchical HMM} for probabilistic sequence modeling
    \item Implement \textbf{Dynamic Bayesian Networks (DBN)} for temporal dependency modeling
    \item Implement \textbf{Markov Decision Processes (MDP)} and \textbf{PO-MDP} for sequential decision-making
    \item Implement \textbf{Markov Random Fields (MRF)} for spatial-temporal dependencies
    \item Implement \textbf{Granger Causality} for causal relationship analysis
...
\begin{frame}{Liquid Time-Constant Network (LTC) - Hasani et al., 2020}
\begin{columns}
\begin{column}{0.5\textwidth}
\textbf{Architecture:}
\begin{itemize}
    \item 2-layer LTC network
    \item Hidden size: 128
    \item Adaptive time constants
    \item Neural ODE dynamics
    \item Euler integration (dt=0.1)
    \item Classification head
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\textbf{Advantages:}
\begin{itemize}
    \item Continuous-time modeling
    \item Adaptive temporal dynamics
    \item Captures fast \& slow patterns
    \item Neural ODE integration
    \item Flexible time scales
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    ...
    \item LTC demonstrates adaptive temporal dynamics through continuous-time modeling
\end{itemize}
\end{frame}
...
\textbf{Metric} & \textbf{FFNN} & \textbf{Trans.} & \textbf{3stage} & \textbf{CNN} & \textbf{LSTM} & \textbf{Hopfield} & \textbf{VAE} & \textbf{LTC} \\
\midrule
Train Time (s) & \textbf{XX.XX} & \textbf{XX.XX} & \textbf{XX.XX} & \textbf{XX.XX} & \textbf{XX.XX} & \textbf{XX.XX} & \textbf{XX.XX} & \textbf{XX.XX} \\
...
\begin{itemize}
    ...
    \item LTC: Moderate speed, continuous-time dynamics
...
\begin{itemize}
    ...
    \item[+] Adaptive temporal
    \item[-] ODE solver overhead
    \item[-] Training complexity
\end{itemize}
\end{column}
\end{columns}
...
\begin{itemize}
    ...
    \item \textbf{LTC}: Continuous-time modeling, adaptive temporal dynamics, varying time scales
\end{itemize}
\end{frame}
...
\midrule
\textbf{Input} & Features & Raw & Raw & Raw & Raw & Raw & Raw & Raw \\
\textbf{Modeling} & None & Global & Multi-scale & Local & Sequential & Energy & Latent & Continuous-time \\
\textbf{Speed} & Fastest & Moderate & Slowest & Fast & Moderate & Moderate & Moderate & Moderate \\
\textbf{Accuracy} & Good & Excellent & Best & Good+ & Good+ & Good+ & Good+ & Good+ \\
\textbf{Explain.} & Moderate & High & High & Moderate & High & Moderate & Highest & Moderate \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Attention} (Transformer/3stageFormer) vs. \textbf{Convolution} (CNN) vs. \textbf{Recurrence} (LSTM) vs. \textbf{Continuous-time ODE} (LTC)
    ...
    \item \textbf{Discrete-time} (most) vs. \textbf{Continuous-time} (LTC)
\end{enumerate}
\end{frame}
...
\begin{enumerate}
    \item All eight architectures achieve \textbf{good performance} on ECG classification
    ...
    \item LTC demonstrates \textbf{adaptive temporal dynamics} through continuous-time modeling
...
\begin{itemize}
    ...
    \item Compare \textbf{ensemble methods} combining all eight models
    ...
    \item Investigate \textbf{LTC adaptive time constants} for varying temporal patterns
\end{itemize}
\end{frame}
...
\begin{enumerate}
    ...
    \item Hasani, Ramin, et al. (2020). "Liquid Time-Constant Networks." \textit{arXiv preprint arXiv:2006.04439}.
